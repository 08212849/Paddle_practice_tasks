...
model = AutoModelForSequenceClassification.from_pretrained("bert-wwm-chinese", num_classes=14)
...
class RobertaForSequenceClassification(paddle.nn.Layer):
    def __init__(self, MODEL_NAME, num_class=14, dropout=None):
        super(RobertaForSequenceClassification, self).__init__()
        # 加载预训练好的Roberta，只需要指定一个名字就可以
        self.Roberta = paddlenlp.transformers.RobertaModel.from_pretrained(MODEL_NAME)
        self.dropout = nn.Dropout(dropout if dropout is not None else self.ernie.config["hidden_dropout_prob"])
        self.classifier = nn.Linear(self.Roberta.config["hidden_size"], num_class)
...
paddle.save(model.state_dict(), "model/{}.pdparams".format(model_name))
paddle.save(optimizer.state_dict(), "model/{}.optparams".format(model_name))
tokenizer.save_pretrained("model/{}_tokenizer".format(model_name))