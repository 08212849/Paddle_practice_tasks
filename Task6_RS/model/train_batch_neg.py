...
parser.add_argument('--log_steps', type=int, default=30, help="Interval steps to print log")
...
parser.add_argument("--similar_text_pair_file", type=str, default='./recall/dev.csv',
...
parser.add_argument("--recall_result_file", type=str, default='recall_result_init.txt',
...
parser.add_argument("--use_gradient_cache", action='store_true',
...
def save_ckpt(model, tokenizer, output_dir, ori_config):
    """
    保存模型、分词器和配置信息的checkpoint到指定目录。

    Args:
        model (paddle.nn.Layer): 需要保存的模型。
        tokenizer (PretrainedTokenizer): 模型的分词器。
        output_dir (str): 保存checkpoint的目录路径。
        ori_config (PretrainedConfig): 模型的原始配置信息。
    ...
    #save config
    ori_config.save_pretrained(output_dir)
    save_config_path = os.path.join(output_dir, "config.json")
    with open(save_config_path, 'r') as json_file:
        config_dict = json.load(json_file)
    config_dict["init_class"] = "ErnieForPretraining"
    with open(save_config_path, 'w') as json_file:
...
def train(train_data_loader, model, optimizer, lr_scheduler, rank,
    corpus_data_loader, query_data_loader, recall_result_file,
    text_list, id2corpus, tokenizer, config
):
    """
    基本的训练循环，直接处理批次，不涉及任何高级技术如分块或梯度缓存。
    不涉及任何关于梯度的显式处理，除了基本的反向传播，并没有管理中间状态或嵌入。
    """
    global_step = 0
    best_recall = 0.0
    tic_train = time.time()
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            if not args.evaluate:
                if global_step % args.save_steps == 0 and rank == 0:
                    ...
                    save_ckpt(model, tokenizer, save_dir, config)
...
def gradient_cache_train(train_data_loader, model, optimizer, lr_scheduler, rank, tokenizer):
    """
    设计用于处理大批量数据，通过将大批量分割成更小的子批量（子批次），分别处理这些子批次，然后聚合结果。
    函数采用了梯度缓存技术，这在内存限制或需要有效管理大规模计算时非常有用。它捕捉并重用中间梯度，以优化内存使用和计算效率。
    使用混合精度训练（paddle.amp）来加速训练，同时保持稳定性所需的精度。
    """
    ...
    if args.batch_size % args.chunk_numbers == 0:
        ...
        raise Exception(
            f" Batch_size {args.batch_size} must divides "
            "chunk_numbers {args.chunk_numbers} without producing a remainder "
        )
...
def do_train():
    ...
    pretrained_model = AutoModel.from_pretrained(args.model_name_or_path)#, recompute_enable=args.use_recompute)
